...
public class FsmProcess {
	...
	static public void generateDot()
	{
		...
		for (int n=0;n<fsm.states.size();n++)
		{
			...
			if (nbTransitions!= 0) 
				for (int m=0;m<nbTransitions;m++)
				{
					...
					bufDot.append("[shape=box, label=  <<TABLE BORDER=\"0\" CELLBORDER=\"1\" CELLSPACING=\"0\" CELLPADDING=\"4\"> <TR>   <TD COLSPAN=\"");
		...
		for (int k=0;k<fsm.resetTransitions.size();k++)
		{
			...
			bufDot.append("  ");
			if (rt.attachedActions.size()==0) //the table and condition have not yet been generated by the first action
			{
				bufDot.append("[style=\"dashed\", shape=box, label=  <<TABLE BORDER=\"0\" CELLBORDER=\"1\" CELLSPACING=\"0\" CELLPADDING=\"4\"> <TR>   <TD COLSPAN=\"");
